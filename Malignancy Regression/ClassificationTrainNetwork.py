# -*- coding: utf-8 -*-
"""
Created on Mon Aug 27 14:37:35 2018
...
This script is used to train the regression model one time
...
path='C:/Users/s120116/Documents/Preprocessed_Images/Crops(16x32x32)Malignancy2/'

cancer_folder=path+'subset0/training/cancer/cancer*/*'
val_cancer_folder=path+'subset0/validate/cancer/cancer*/*'


crop_size=np.array([32,64,64])
cnn=model.malignancy_classification(input_shape=(32,64,64,1))
...
#train network
i=0
Iterations=1000#int(180000 / cancer_batchsize) #nr of iterations for one epoch for all scales
...
for i in range(Iterations):#int(np.around(len(cancer_trainset)*n_epochs/cancer_batchsize))):
    try:
        ...
        im_train,lab_train = get_sample_from_batches(cbatch)
...
import keras
cnn=keras.models.load_model('C:/Users/s120116/Documents/classification/TrainingData/neuralnet_final.h5')
#
...
errorlist_train=get_predictions(sample_cancer_train, cancer_trainset, cancer_batchsize)
errorlist_val=get_predictions(sample_cancer_val, cancer_testset, cancer_batchsize)
...
#
#np.save(savepath+ '/error', accuracy_val_pos)
#
#
##
#
