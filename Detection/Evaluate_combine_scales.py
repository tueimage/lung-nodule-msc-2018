...
def verify_predictions(prediction_im,bounding_mask,bounding_segm, treshold,FP_correction=True):
      ...
      if FP_correction==True:
          for i in FalsePositive: #ga elk label langs van false positive
              if any (bounding_segm[label_prediction==i]==1): #als er overlap is met een irrelevant finding
                  FP_copy.remove(i)
...
def eval_on_images(path,cnn, nodules_df, nodules_eval, crop_size = np.array([16,32,32]), step_size = np.array([8,8,8]),FPminingNeed=False):
    ...
    nodules_eval['diameter_mm']=nodules_eval['diameter_mm'].abs()
    
    #nodules_eval['diameter_mm']=nodules_eval['diameter_mm'].replace(-1,10)
    ...
    treshold_list=[  0.7, 0.8, 0.85, 0.9, 0.93, 0.95, 0.97,0.99,0.995,0.998,1]
...
def main():
    ...
    path='/home/lshesse/evaluate18-07/Correct_Scripts/Runs_C3DNet/Increased_Neurons/'
